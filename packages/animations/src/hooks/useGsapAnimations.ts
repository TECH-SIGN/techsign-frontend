...
import type { UseGsapAnimationsOptions } from "@techsign/shared";
      ...
      try {
        ...
        const ctx = gsap.context(() => {
          ...
          scrubVideos.forEach((wrap) => {
            ...
            const overlay = wrap.querySelector(
              ".hero-video-overlay",
            ) as HTMLElement | null;
        ...
        // Avoid depending on Vite-specific typing in package build
        try {
          (console as any).warn?.("[useGsapAnimations] gsap not installed", e);
        } catch {}
      }
    })();
